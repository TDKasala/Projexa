            ...
            {invoices.map((invoice) => (
              <Tr key={invoice.id}>
                <Td className="font-medium">
                  <Link href={`/facturation/${invoice.id}`} className="hover:underline">
                    {invoice.invoice_number ?? `DOC-${invoice.id.slice(0, 8).toUpperCase()}`}
                  </Link>
                </Td>
                ...
                <Td>{Number(invoice.total).toLocaleString("fr-FR")} FC</Td>
                <Td className="text-right">
                  <div className="flex justify-end gap-4">
                    <Link href={`/facturation/${invoice.id}`} className="text-sm font-medium text-blue-600 hover:underline">
                      Voir
                    </Link>
                    <Link href={`/facturation/${invoice.id}/modifier`} className="text-sm font-medium text-navy-950 hover:underline">
                      Modifier
                    </Link>
                    <DeleteButton
